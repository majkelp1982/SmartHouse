...
public class Module_Weather extends Module implements Cloneable {
    ...
    private transient static byte MODULE_TYPE = 1;
    ...
    public boolean compare(Module_Weather module_weather) {
        ...
        boolean result = true;
        if (result) result = cmp(module_weather.bme280.getTemp(), bme280.getTemp(), 1);
        if (result) result = cmp(module_weather.bme280.getPressure(), bme280.getPressure(), 5);
        if (result) result = cmp(module_weather.bme280.getHumidity(), bme280.getHumidity(), 3);
